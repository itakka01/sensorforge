...
// Small bridge exported by webconfig.cpp. This keeps the reader module
// independent from the rest of WebConfig while preserving the same page shell.
extern String webConfigLogReaderHtmlHeader();
extern String webConfigLogReaderHtmlFooter();
extern String webConfigLogReaderHtmlEscape(const String &value);
extern bool webConfigLogReaderRejectWhileRecording(const char *operation);

static WebServer *gWebLogReaderServer = nullptr;
#define server (*gWebLogReaderServer)
...
{
    if (webConfigLogReaderRejectWhileRecording("log download"))
        return;
...
{
    if (webConfigLogReaderRejectWhileRecording("log live update"))
        return;
...
{
    if (webConfigLogReaderRejectWhileRecording("log clear"))
        return;
...
{
    if (webConfigLogReaderRejectWhileRecording("log viewer"))
        return;
    ...
    html +=
        ...
        webConfigLogReaderHtmlEscape(logAnalyzeButtonText) +
        "</button>"
        "<div id='logAnalysisStatus' class='log-analysis-status'>" +
        webConfigLogReaderHtmlEscape(logAnalysisIdleText) +
        "</div></div></div>"
    ...
    html +=
        ...
        "<div class='log-meta'>"
        "<span>Datei: <code>" +
        webConfigLogReaderHtmlEscape(logPath) +
        "</code></span>"
        "<span>Größe: <span id='logSize'>" +
        webConfigLogReaderHtmlEscape(sizeText) +
        "</span></span>"
    ...
    html += webConfigLogReaderHtmlFooter();
...
void webLogReaderRegisterRoutes(WebServer &webServer)
{
    gWebLogReaderServer = &webServer;

    webServer.on("/log", HTTP_GET, handleLog);
    webServer.on("/log_raw", HTTP_GET, handleLogRaw);
    webServer.on("/log_chunk", HTTP_GET, handleLogChunk);
    webServer.on("/log_clear", HTTP_POST, handleLogClear);
}
